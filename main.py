...
class ProcessOutputCallback:
    ...
    def process_output(self, latent_image, progress_step=100, finished=True, error=None, message=None):     
        if error:
            print('error')
            self.api_worker.send_progress(100, None, self.job_data)
            ...
            if not finished:
                ...
                if self.api_worker.progress_data_received:
                    ...
                    return self.api_worker.send_progress(progress_info, progress_data, self.job_data)
            else:
...
def load_flags():
    ...
    parser.add_argument(
        "--model", type=str, default="models/sd3.5_large.safetensors", help="Model weights"
                        )
    parser.add_argument(
...
def main():
    ...
    inferencer.load(Path(args.model), None, 3.0)
    callback = ProcessOutputCallback(api_worker, inferencer, 'stable-diffusion-3.5-large')
        ...
        try:
            ...
            if init_image:
                if not isinstance(init_image, str):
                    raise ValueError(f"Expected a base64 string for init_image, got {type(init_image)} with value {init_image}")
            ...
            inferencer.gen_image(
                ...
                # controlnet_cond_image=job_data.get('controlnet_cond_image'),
